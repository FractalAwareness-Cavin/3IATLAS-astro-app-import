...
2. Run the script that matches your OS:
   ...
   - macOS: double-click `MacOS_SEEREADME_KStars_Append_3I.command`. If Gatekeeper warns “Apple could not verify…”, go to **System Settings → Privacy & Security**, set **Allow applications downloaded from** to *App Store & identified developers*, launch the script once (click **Done** in the warning), then press **Open Anyway** in Privacy & Security and relaunch. When the dialog suggests **Move to Trash**, remember it is just macOS flagging an unsigned download—choose **Cancel**, approve the helper via **Open Anyway**, and run it again. The follow-up prompt for Touch ID or your account password belongs to macOS; the script never sees or stores any credentials.
