...
Files by platform
-----------------
- `KStars_Append_3I.command` — macOS helper (auto-detects `~/Library/Application Support/kstars/comets.dat`).
- `KStars_Append_3I.sh` — Linux helper.
- `KStars_Append_3I-DRYRUN.bat` — Windows preview wrapper.
- `KStars_Append_3I-APPLY.bat` — Windows apply wrapper.
- `KStars_Append_3I.ps1` — PowerShell backend invoked by both batch files.
...
2. Run `KStars_Append_3I.command`. When the preview appears, type `y` and press **Enter**. Gatekeeper may require you to click **Done** and then **Open Anyway** once.

Linux
-----
1. Run `bash tools/KStars_Append_3I.sh --apply` (or invoke `append_line.sh` directly).

Windows
-------
1. Run `KStars_Append_3I-DRYRUN.bat` to preview.
2. Run `KStars_Append_3I-APPLY.bat` to write the change.
